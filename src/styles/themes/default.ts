export const defaultTheme = {
    colors: {
        "base-white": "#FFFFFF",
        "base-background": "#FAFAFA",
        "base-card": "#F3F2F2",
        "base-input": "#EDEDED",
        "base-button": "#E6E5E5",
        "base-hover": "#D7D5D5",
        "base-label": "#8D8686",
        "base-text": "#574F4D",
        "base-subtitle": "#403937",
        "base-title": "#272221",
        "base-error": "#f84747",
        "brand-purple-dark": "#4B2995",
        "brand-purple": "#8047F8",
        "brand-purple-light": "#EBE5F9",
        "brand-yellow-dark": "#C47F17",
        "brand-yellow": "#DBAC2C",
        "brand-yellow-light": "#F1E9C9",
      },
      textSizes: {
        "title-title-xl": "3rem",
        "title-title-lg": "2rem",
        "title-title-md": "1.5rem",
        "title-title-sm": "1.2rem",
        "title-title-xs": "1.125rem",
        "text-regular-lg": "1.25rem",
        "text-bold-lg": "1.25rem",
        "text-regular-md": "1rem",
        "text-bold-md": "1rem",
        "text-regular-sm": "0.875rem",
        "text-bold-sm": "0.75rem",
        "components-tag": "0.625rem",
        "components-button-g": "0.875rem",
        "components-button-sm": "0.75rem",
      },
      fonts: {
        regular: "'Roboto'",
        title: "'Baloo 2'",
      },
}